'''
Script to generate galaxy random catalog
'''
...
### geometry files
geom_boss_total_fname = mask_boss_fdir + "boss_geometry_2014_05_28.ply"
geom_boss_lowz_fname = mask_boss_fdir + "mask_DR12v5_LOWZ_North.ply"
...
geom_tot = pymangle.Mangle(geom_boss_total_fname)
geom_boss_lowz = pymangle.Mangle(geom_boss_lowz_fname)
...
logger.info("Generate random RA DEC and apply survey geometry cut")

galcone_tot = []
part_num = [] # cumulative number

gal_num = int(1e6)
galcone_ori = np.empty((int(gal_num),), dtype=fgal_type)
galcone_ori['ra'], galcone_ori['dec'] = geom_tot.genrand(gal_num)
galcone_ori['w'] = geom_tot.weight(galcone_ori['ra'], galcone_ori['dec'])
galcone_ori = galcone_ori[galcone_ori['w'] > 0]

for igeom, geometry in enumerate(geoms):
    select = geometry.contains(galcone_ori['ra'], galcone_ori['dec'])
    galcone = galcone_ori[select]
...
for ipart, nz_boss_fname in enumerate(nz_boss_fname_list):
    ...
    if ipart != 0:
        tmp_weight = geom_boss_lowz.weight(galcone_dsampled['ra'], galcone_dsampled['dec'])
        select = (tmp_weight == 0)
        galcone_dsampled = galcone_dsampled[select]
