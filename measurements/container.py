...
class SurveyData:
    ...
    def apply_condition_cut(self, cond, in_place=False):
        ''' 
        apply condition cut. The condition should be a boolean array 
        that has the same length as data.    
        '''

        assert len(cond) == len(self.ra)

        if in_place:
            for iattr in self.colnames:
                ival = getattr(self, iattr)
                setattr(self, iattr, ival[cond])
            return None
    ...
    def match_to_reference(self, ref_cat:"SurveyData", nside_x=64, nside_y=64):
        match_selected = self._get_matched_condition(ref_cat, nside_x, nside_y)
        self.apply_condition_cut(match_selected, cached=False)
    ...
    def _get_matched_condition(self, ref_cat:"SurveyData", nside_x=64, nside_y=64):
