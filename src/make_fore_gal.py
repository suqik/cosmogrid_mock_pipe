...
nhod_per_cosmo = 1 # Number of varied HOD parameter values per cosmology
...
''' main routines '''
def make_hod_halocat(cosmo_label):
...
def find_fic(halo_mass, hod_param_vals):
    if model == 2:
        if isinstance(hod_param_vals, list):
...
def find_hod_params_alive(cosmo_label, halo_mass, hod_param_file, num_pool=10000, seedini=9782):
    ...
    #         dict_of_gsamples = apply_hod(halo_file, hod_halo_cat, OmegaM, model_params_dict)
    ...
    #             # logger.info(f"Save snapshot data to file: {out_dir + out_fmt.format(cosmo_label, ihod, iseed, "snap")}")
    #             # np.savetxt(out_dir + out_fmt.format(cosmo_label, ihod, iseed, "snap"),
    #             #         gal_pos, fmt="%.3f %.3f %.3f")
            
    #         gsamples_list.append(dict_of_gsamples)
                
    #     return gsamples_list
    # else:
    #     return None

def make_survey(gsamples_list:list, masks:dict, cosmo_ccl:ccl.Cosmology, nofz_info:dict, check_repeat:bool=False, cosmo_label:int=0, outname:str="lcone"):
    assert isinstance((gsamples_list), list)
    ...
    for ihod, dict_of_gsamples in enumerate(gsamples_list):
        for iseed, key in enumerate(dict_of_gsamples.keys()):
            ...
            logger.info("Save lightcone data to file.")

            np.savetxt(out_dir + out_fmt.format(cosmo_label, ihod, iseed, outname), galcone_output, fmt="%.4f %.4f %.4f %4f")
...
if __name__ == "__main__":
    ...
    for cosmo_label in range(1,2):
        ...
        hod_param_file = "cfgs/cosmo_{:06d}_hod_params_alive.txt".format(cosmo_label)

        ### load halo catalog
        halo_file, hod_halocat, OmegaM = make_hod_halocat(cosmo_label)
        # hod_params_alive = find_hod_params_alive(cosmo_label, hod_halocat.halo_mvir, hod_param_file, num_pool=10000, seedini=9782)
        ...
        #     make_survey(gsamples_list, masks, cosmo_ccl, nofz_info, check_repeat=False, cosmo_label=cosmo_label, outname="boss_north_2dflens_south")
